(()=>{
  ...
  const load=src=>new Promise((ok,no)=>{const s=document.createElement('script');s.src=`${src}?v=20260914-25`;s.onload=ok;s.onerror=no;document.head.appendChild(s)});
  const boot=async()=>{
    ...
    try{await load('skills.js')}catch(e){console.warn('NOVEN skills layer failed',e)}
  };
