const API_BASE = localStorage.getItem('nashhal_ai_api') || '';
...
form.addEventListener('submit', async (event) => {
  ...
  try {
    ...
    answer.textContent = 'واجهة النموذج جاهزة، لكن خدمة API غير متصلة حاليًا. اضبط عنوان API في إعدادات البيئة أو localStorage ثم أعد المحاولة.';
    renderSources([]);
    console.error(error);
  } finally {
    send.disabled = false;
