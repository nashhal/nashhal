(() => {
  ...
  const builtIn = [
    ...
    {
      keys: ['what is ai', 'what is artificial intelligence', 'artificial intelligence'],
      title: 'Artificial intelligence',
      source: 'Reference',
      url: 'https://en.wikipedia.org/wiki/Artificial_intelligence',
      text: 'Artificial intelligence is the field of building computer systems that can perform tasks commonly associated with human intelligence, including learning, reasoning, perception, language understanding, and decision-making.'
    },
    {
      ...
      source: 'NOVEN',
      url: './#web3',
      text: 'NOVEN uses Web3 as an optional trust layer. A result can be hashed locally and, with a compatible wallet, signed for verification. Blockchain is used here for provenance rather than for inference.'
    },
    {
      keys: ['github pages', 'gpu', 'local model', 'qwen'],
      title: 'NOVEN architecture',
      source: 'NOVEN',
      url: './#system',
      text: 'GitHub Pages serves the static interface. A full language model needs compute outside static hosting, so NOVEN combines browser retrieval with public references and keeps a path open for a self-hosted model.'
    },
    {
      keys: ['retrieval', 'search', 'source', 'sources', 'evidence'],
      title: 'Retrieval and evidence',
      source: 'NOVEN',
      url: './#research',
      text: 'NOVEN ranks available knowledge records against the question, selects the strongest matches, and keeps supporting evidence attached to the response.'
    },
    {
      keys: ['hello', 'hi', 'مرحبا', 'السلام'],
      title: 'NOVEN',
      source: 'NOVEN',
      url: './',
      text: 'Hello. I’m NOVEN. Ask a question and I’ll search the available knowledge and public reference sources.'
    }
  ];
  ...
  async function loadKnowledge() {
    for (const path of ['../data/live-news.json', '../data/demo-kb.json']) {
      try {
        ...
        console.warn('NOVEN knowledge source unavailable:', path, error);
      }
    }
    return [];
  }
  ...
  async function publicReference(q) {
    ...
    const page = `https://${language}.wikipedia.org/wiki/${encodeURIComponent(first.title.replace(/ /g, '_'))}`;
    return {
      title: first.title,
      source: 'Wikipedia',
      url: page,
      text: `A public reference for “${first.title}” was found. Open the source to read the full article.`,
      score: 1
    };
  }
  ...
  async function answerQuestion(q) {
    ...
    if (ranked.length) {
      const lead = ranked[0];
      return {
        text: `${lead.text || lead.description || lead.title}\n\nNOVEN found matching evidence in its current knowledge layer.`,
        sources: ranked
      };
    }
    ...
    try {
      ...
      if (ref) return {
        text: `${ref.text}\n\nReference: ${ref.title} (Wikipedia).`,
        sources: [ref]
      };
    } catch (error) {
      console.warn('NOVEN public reference lookup unavailable:', error);
    }

    return {
      text: 'I could not retrieve a reliable public reference for that question right now. The browser runtime is active; try a more specific question.',
      sources: []
    };
  }
  ...
  async function optionalApi(q) {
    ...
    try {
      ...
      console.warn('NOVEN optional AI backend unavailable:', error);
      return null;
    }
  }
